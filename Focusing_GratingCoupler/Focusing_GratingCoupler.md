...
聚焦光栅接受到的入射光通常由阶跃型单模光纤传出，该入射光离开光纤后直接进入光栅结构顶部的玻璃包层。由于阶跃型单模光纤的出射光可以近似为高斯光源的场分布，所以在本案例的仿真工程中不对阶跃型单模光纤部分进行建模，仅将光纤的出射光用高斯光束代替。因此，在软件中设置高斯光源时，需要注意：
- 高斯光源的束腰尺寸与实际使用的光纤纤芯尺寸匹配；
- 高斯光源束腰面位置即代表光纤与光栅顶部包层的接触面位置，需要将光源所在的背景材料设置为*Silica*（玻璃）。
