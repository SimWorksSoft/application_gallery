...
威尔金森功分器的S参数如下图所示，其中心频率为$0.966GHz$，与设计频率$1GHz$相差小于4%。在频率为$1GHZ$处， $S_{11}=-27dB$、$S_{22}=-30dB$，说明该器件在设计频率下，从任意端口输入得到的反射都非常小，表明端口之间的阻抗匹配良好。$S_{32}=-33dB$，说明器件从$Port2$输入传输到$Port3$端口的功率非常小，体现了输出端口之间具有良好的隔离性能；$S_{31}=-3.05dB$，且在整个仿真频段内变化小于10%，说明从输入端口（$Port 1$）传输到$Port 3$的功率，在全波段均大约为50%。由于器件的对称性，输入端口传输到$Port 2$的功率也为50%，体现了器件的等功率分配以及良好的带宽。
